...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        if self.randomize_start:
            rand_start = np.random.randint(1, CHUNKS // 2)
            self.helipad_x1 = chunk_x[rand_start - 1]
            self.helipad_x2 = chunk_x[rand_start + 1]
            self.helipad_y = H / 4
            height[rand_start - 2] = self.helipad_y
            height[rand_start - 1] = self.helipad_y
            height[rand_start + 0] = self.helipad_y
            height[rand_start + 1] = self.helipad_y
            height[rand_start + 2] = self.helipad_y
        else:
    ...
    def step(self, action):
        ...
        lander_x = (pos.x - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2)
        lander_y = (pos.y - (self.helipad_y + LEG_DOWN / SCALE)) / (VIEWPORT_H / SCALE / 2)

        helipad_x1_cor = (self.helipad_x1 - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2)
        helipad_y_cor = (self.helipad_y - VIEWPORT_H / SCALE / 2) / (VIEWPORT_H / SCALE / 2)
        helipad_x2_cor = (self.helipad_x2 - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2)

        new_x1 = helipad_x1_cor - lander_x
        new_y1 = helipad_y_cor - lander_y
        new_x2 = helipad_x2_cor - lander_x
        new_y2 = new_y1
